...
- **图片生成** (`image`) —— 支持尺寸/质量/风格配置，自动下载保存
...
# 图片生成
dmxapi image "一只在月球上骑自行车的猫" --size 1024x1024
...
**发布内容**（由 `package.json` 的 `files` 字段控制）：

只发布 `dist/` 目录（编译后的 JS + 类型声明），不包含源码、测试、配置文件等。
具体包含：
- `dist/index.js` —— 编译后的入口文件（含 shebang `#!/usr/bin/env node`）
- `dist/index.d.ts` —— TypeScript 类型声明
- `package.json` —— 包元信息
- `README.md` —— 说明文档

不会发布：`src/`、`node_modules/`、`docs/`、`tsconfig.json`、`tsup.config.ts` 等开发文件。
