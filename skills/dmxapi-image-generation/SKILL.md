...
description: 使用 DMXAPI 平台生成和编辑图片。支持 Gemini、Seedream（豆包即梦）、OpenAI 等多种模型。可进行文生图、图片编辑、联网搜索增强生图。当用户需要生成图片、编辑图片、AI 绘图时使用此技能。
...
| `-n, --count <n>` | 生成数量 | `-n 3` |
| `--image <path>` | 输入图片路径（编辑模式） | `--image ./photo.png` |
